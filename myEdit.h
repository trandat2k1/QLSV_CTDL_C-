...
int DrawMenu(int vt_item, int vt_y)
{
	...
	for (i = 0; i < so_item; i++)
	{
		rectagle(cot, dong + i * 7, 20,4);
		gotoxy(cot+4, dong + i * 7+2);
		cout << td[i];
	}
	HighLight();
	rectagle(cot, dong + vt_y, 20, 4);
	gotoxy(cot + 4, dong + vt_y + 2);
	cout << td[vt_item];
	int chon = vt_item;
	char kytu;
	do {
		...
		{
			Normal();
			rectagle(cot, dong + chon * 7, 20, 4);
			gotoxy(cot + 4, dong + chon * 7 + 2);
			cout << td[chon];
			chon--;
			HighLight();
			rectagle(cot, dong + chon * 7, 20, 4);
